...
```bash
git clone https://github.com/YOUR_USERNAME/sparse-autoencoder
...
```bash
# Build the prompt-family dataset
python scripts/run_dataset.py --config configs/default.yaml

# Extract SAE features for all prompts (downloads GPT-2 + SAE on first run, ~650 MB)
python scripts/run_feature_extraction.py --config configs/default.yaml

# Evaluate paraphrase consistency from saved features
python scripts/run_consistency_eval.py --config configs/default.yaml

# Evaluate behavior-feature associations (runs GPT-2 predictions)
...
The full pipeline runs on CPU. Steps 1, 3, 4, and 5 are fast (under a minute each). Step 2 takes roughly 5–15 minutes depending on your machine.
...
sparse-autoencoder/
├── configs/
│   └── default.yaml              # Model, SAE, and hyperparameter settings
├── data/
│   └── processed/
│       ├── prompt_families.json  # 16 factual-recall families with paraphrases
│       └── features.json         # Extracted SAE features for all 112 prompts
├── reports/
│   ├── consistency_results.csv   # Per-family Jaccard overlap statistics
│   ├── behavior_results.json     # Correct/incorrect feature association data
│   └── figures/                  # Generated PNG figures
├── src/
│   ├── data/                     # Prompt family definitions and dataset builder
│   ├── models/                   # Model loading and forward-hook utilities
│   ├── features/                 # SAE loading, feature extraction, and statistics
│   ├── evaluation/               # Consistency and behavior analysis
│   ├── visualization/            # Plot generation
│   └── utils/                    # Seed, I/O helpers
├── scripts/                      # Five CLI scripts, one per pipeline step
├── notebooks/
